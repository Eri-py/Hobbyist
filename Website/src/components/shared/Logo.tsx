...
export function Logo({ width, color }: { width: string; color?: string }) {
  return (
    <svg
...
export function LogoWithName({ size, align, color }: LogoWithNameProps) {
  ...
  switch (size) {
    case "small":
      return logoWithName("18px", 16, color);
    case "medium":
      return logoWithName("27px", 22, color);
    case "large":
      return logoWithName("45px", 40, color);
  }
}
